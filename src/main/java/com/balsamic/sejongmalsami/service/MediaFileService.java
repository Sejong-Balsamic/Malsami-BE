...
public class MediaFileService {
  ...
  // 답변 파일 저장
  @Transactional
  public MediaFileDto uploadMediaFileForAnswerPost(MediaFileCommand command) {
    AnswerPost answerPost = answerPostRepository.findById(command.getQuestionPostId())
        .orElseThrow(() -> new CustomException(ErrorCode.ANSWER_POST_NOT_FOUND));

    String extensionType = s3Service.getExtensionType(command.getFile());

    // 첨부파일이 3개를 초과하는지 체크
    if (mediaFileRepository.countByAnswerPost(answerPost) > MAX_MEDIA_FILE_COUNT) {
      throw new CustomException(ErrorCode.MEDIA_FILE_LIMIT_EXCEEDED);
    }
